// SPDX-License-Identifier: MIT
// Copyright © 2018 WireGuard LLC. All Rights Reserved.

import Foundation
import os.log

extension FileManager
{
    public static var sharedFolderURL: URL?
    {
        guard let appGroupId = Bundle.main.object(forInfoDictionaryKey: "org.OperatorFoundation.Moonbounce.MacOS") as? String else
        {
            os_log("Cannot obtain app group ID from bundle", log: OSLog.default, type: .error)
            return nil
        }
        
        guard let sharedFolderURL = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: appGroupId) else
        {
            //wg_log(.error, message: "Cannot obtain shared folder URL")
            return nil
        }
        
        return sharedFolderURL
    }

    public static var networkExtensionLogFileURL: URL?
    {
        return sharedFolderURL?.appendingPathComponent("tunnel-log.bin")
    }

    public static var networkExtensionLastErrorFileURL: URL?
    {
        return sharedFolderURL?.appendingPathComponent("last-error.txt")
    }

    public static var appLogFileURL: URL?
    {
        guard let documentDirURL = FileManager.default.urls(for: .cachesDirectory, in: .userDomainMask).first else
        {
            //wg_log(.error, message: "Cannot obtain app documents folder URL")
            return nil
        }
        
        return documentDirURL.appendingPathComponent("app-log.bin")
    }

    public static func deleteFile(at url: URL) -> Bool
    {
        do
        {
            try FileManager.default.removeItem(at: url)
        }
        catch
        {
            return false
        }
        
        return true
    }
}


